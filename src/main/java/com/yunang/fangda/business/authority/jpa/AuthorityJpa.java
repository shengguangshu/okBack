...
//    ORDER BY ?#{#pageable}
    @Query(value = "select j.uuid,j.jur_name jurName,j.jur_flag jurFlag,j.jur_parent jurParent,j.jur_type jurType from authority_table a left join jurisdiction_table j on j.uuid = a.aut_jur_id where a.aut_pos_id = ?1 order by j.uuid",
            nativeQuery = true)
    List<Object[]> findJurisdictionModelByAutPostId(String autPosId);

    AuthorityModel findByAutPostIdAndAutJurId(String autPostId, String autJurId);
